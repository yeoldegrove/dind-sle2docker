...
if [ $? -eq 0 ]; then
    echo "$(tput setaf 1)opensuse image for sle2docker created$(tput sgr 0)" && \
    echo "$(tput setaf 1)to build a SLE container, use it like:$(tput sgr 0)" && \
    echo "$(tput setaf 1)# docker run -t -i --privileged sle2docker sle2docker build -s <repo IP/url> (--disable-https) SLE11SP2|SLE11SP3|SLE12$(tput sgr 0)"
else
